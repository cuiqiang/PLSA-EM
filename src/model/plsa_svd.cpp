...
void PLSA_SVD::init(){
   ...
   cout << "init p(w|z),p(d|z),p(z)" << endl;

}

void PLSA_SVD::E_step()// assign p(z|w,d)
{
	for(int w=0; w<row; w++){
		for(int d=0; d<column; d++){
			double norm = 0.0;
			for(int z=0; z<K; z++){
				double val = Pw_z[z][w]*Pd_z[z][d]*Pz[z];
				Pz_wd[w][d][z] = val;
				norm += val;
			}
			for(int z=0; z<K; z++){
...
double PLSA_SVD::calcLogLiklihood()
{
	double loglik = 0.0;
	for(int w=0; w<row; w++){
		for(int d=0; d<column; d++){
...
void PLSA_SVD::var_infer(int step_count, double alpha, double beta, double threshold)
{
    init();
    double L = 0.0;
...
void PLSA_SVD::debug_model_para(){
	for(int w=0; w<row; w++){
		for(int z=0; z<K; z++){
			cout << Pw_z[z][w] << SPACE;
		}
...
void PLSA_SVD::model_output(char* modelFile){
...
void PLSA_SVD::train(char* trainData){
...
void PLSA_SVD::load_corpus(char* trainData){
